...
def _sec_pmt(lang: str = "it") -> dbc.AccordionItem:
    return _section(t("met.s1.title", lang),
                    ...
                    html.Div("Rata = M × r_m / (1 − (1 + r_m)^(−n))",
                             style=_FORMULA),
                    html.Ul([
                        html.Li(
                            [_var("M"), " = capitale erogato = Prezzo − Anticipo"]),
                        html.Li(
                            [_var("r_m"), " = tasso mensile = Tasso annuo / 12"]),
                        html.Li(
                            [_var("n"), " = numero rate = Durata (anni) × 12"]),
                    ], className="small mb-2"),
                    html.P([
                        "Ogni rata è composta da ", html.Strong(
                            "interessi"), " (saldo residuo × r_m) e ",
                    ...
                    dbc.Table([
                        ...
                        html.Tbody([
                            html.Tr([
                                ...
                                html.Td(
                                    html.Code("Affitto_lordo × 12 / Prezzo")),
                                html.Td(
                                    "Rendimento lordo nominale del canone, senza costi né tasse"),
                            ]),
                            html.Tr([
                                html.Td(html.Strong("Cap Rate")),
                                html.Td(html.Code("NOI_pretax / Prezzo")),
                                html.Td(
                                    "Reddito operativo netto ante-imposte canone sul valore dell'immobile"),
                            ]),
                            html.Tr([
                                html.Td(html.Strong("Net Yield")),
                                html.Td(html.Code("NOI_posttax / Prezzo")),
                                html.Td(
                                    "Reddito operativo netto post-imposte canone sul valore dell'immobile"),
                            ]),
                            html.Tr([
                                html.Td(html.Strong("Cash-on-Cash")),
                                html.Td(html.Code("CF_netto_annuo / C0")),
                                html.Td(
                                    "Rendimento effettivo sul capitale totale investito (anno 1)"),
                            ]),
                        ]),
                    ], size="sm", bordered=True, responsive=True, className="small"),
                    html.Div(
...
def _sec_estinzione(lang: str = "it") -> dbc.AccordionItem:
    return _section(t("met.s10.title", lang),
                    html.P([
                        ...
                        html.Em(
                            "\"conviene versare il saldo residuo del mutuo all'anno X, oppure investire quella liquidità?\""),
                    ], className="small"),
